...
import {
  ViewportDto,
  BulkSeatStatusUpdateDto,
} from './dto/create-venue-layout.dto';
import {
  SeatLockRequestDto,
  SeatLockReleaseDto,
  SeatLockExtendDto,
...
export class VenueLayoutController {
  ...
  async create(
    ...
    if (req.user && req.user.role === 'VENUE_OWNER') {
      ...
      const userProfile =
        await this.venueLayoutService.getVenueOwnerProfileByUserId(
          req.user.userId,
        );
      if (!userProfile) {
  ...
  async findOne(@Param('id') id: string, @Req() req: any) {
    ...
    if (req.user && req.user.role === 'VENUE_OWNER') {
      ...
      const canEdit = await this.venueLayoutService.checkOwnerEditPermission(
        id,
        req.user.userId,
      );
  ...
  async update(
    @Param('id') id: string,
    @Body() updateVenueLayoutDto: CreateVenueLayoutDto,
    @Req() req: any,
  ) {
  ...
  async duplicate(
    ...
    if (req.user && req.user.role === 'VENUE_OWNER') {
      ...
      if (!canEdit) {
        throw new BadRequestException(
          'You do not have permission to duplicate this layout',
        );
      }
    }
  ...
  async remove(@Param('id') id: string, @Req() req: any) {
    ...
    if (req.user && req.user.role === 'VENUE_OWNER') {
      ...
      if (!canEdit) {
        throw new BadRequestException(
          'You do not have permission to delete this layout',
        );
      }
    }
  ...
  @Post(':id/viewport')
  getLayoutByViewport(@Param('id') id: string, @Body() viewport: ViewportDto) {
  ...
  updateSeatStatuses(
    ...
    return this.venueLayoutService.updateSeatStatuses(
      layoutId,
      updateDto.updates,
    );
  }
  ...
  bulkUpdateSeats(
    ...
    return this.venueLayoutService.bulkUpdateSeats(
      layoutId,
      body.seatIds,
      body.updates,
    );
